...
apply_config (struct c3x_config * cfg)
{
	...
	/*
	WITH_MEM_PROTECTION (ADDR_BYPASS_UNIT_LISTING_FOR_MENU, 2, PAGE_EXECUTE_READWRITE) {
		byte normal[2] = {0x0F, 0x8E}; // jz [32-bit offset]
		byte modded[2] = {0x90, 0xE9}; // nop; jmp [offset]
		for (int n = 0; n < 2; n++)
			((byte *)ADDR_BYPASS_UNIT_LISTING_FOR_MENU)[n] = cfg->group_units_on_right_click_menu ? modded[n] : normal[n];
	}
	*/
...
/*
void __fastcall
patch_Main_Screen_Form_open_right_click_unit_menu (Main_Screen_Form * this, int edx, int tile_x, int tile_y, int mouse_x, int mouse_y)
{
	if (is->current_config.group_units_on_right_click_menu) {
		unsigned dups_size = 0x100 * sizeof is->unit_menu_duplicates[0]; // Context menus can only store up to 0x100 items
		if (is->unit_menu_duplicates == NULL)
			is->unit_menu_duplicates = malloc (dups_size);
		memset (is->unit_menu_duplicates, 0, dups_size);
	}
	Main_Screen_Form_open_right_click_unit_menu (this, __, tile_x, tile_y, mouse_x, mouse_y);
}
*/
...
patch_Context_Menu_open (Context_Menu * this, int edx, int x, int y, int param_3)
{
	...
	if (is->current_config.group_units_on_right_click_menu &&
		...
		int original_item_count = this->Item_Count;
		for (int n = 0; n < original_item_count; n++) {
			// Skip over this item if it has been flagged for removal, that must have happened b/c it's a duplicate of a previous unit.
			if (removals[n])
				continue;

			Context_Menu_Item * item = &this->Items[n];
			int unit_id = item->Menu_Item_ID - (0x13 + p_bic_data->UnitTypeCount);
			Unit_Body * unit_body;
			if ((unit_id >= 0) &&
			    (NULL != (unit_body = p_units->Units[unit_id].Unit)) &&
			    (unit_body->UnitTypeID >= 0) && (unit_body->UnitTypeID < p_bic_data->UnitTypeCount) &&
			    (p_bic_data->UnitTypes[unit_body->UnitTypeID].Transport_Capacity == 0) &&
			    (unit_body->Container_Unit < 0)) {

				int duplicate_count = 0;
				for (int k = n + 1; k < original_item_count; k++) {
					Context_Menu_Item * dup_item = &this->Items[k];
					int dup_unit_id = dup_item->Menu_Item_ID - (0x13 + p_bic_data->UnitTypeCount);
					Unit_Body * dup_body;
					if ((dup_unit_id >= 0) &&
					    (NULL != (dup_body = p_units->Units[dup_unit_id].Unit)) &&
					    (unit_body->UnitTypeID == dup_body->UnitTypeID) &&
				...
				if (duplicate_count > 0) {
					unsigned new_text_len = strlen (item->Text) + 10;
					char * new_text = civ_prog_malloc (new_text_len);
					snprintf (new_text, new_text_len, "%dx %s", duplicate_count + 1, item->Text);
					new_text[new_text_len - 1] = '\0';
					civ_prog_free (item->Text);
					item->Text = new_text;
					Context_Menu_widen_for_text (this, __, new_text);
		...
		// Free memory in items we're going to remove
		for (int n = 0; n < original_item_count; n++)
			if (removals[n]) {
				if (this->Items[n].Text != NULL) {
					civ_prog_free (this->Items[n].Text);
					this->Items[n].Text = NULL;
				}
				if (this->Items[n].field_4 != NULL) {
					civ_prog_free ((void *)this->Items[n].field_4);
					this->Items[n].field_4 = (int)NULL;
				}
				this->Items[n].Menu_Item_ID = -1;
				this->Items[n].Status = 0;
			}
		...
		this->Item_Count = k;
		this->separator_item_count = k; // Not sure what this field holds vs. Item_Count but after the menu is set up they contain the same
		// value and keeping them that way works.
		...
		*/
		...
		int unit_start_index = -1;
		for (int n = 1; n < this->Item_Count; n++)
			if ((this->Items[n].Status == 5) && (this->Items[n - 1].Status == 5)) {
				unit_start_index = n;
				break;
			}

		if (unit_start_index >= 0) {
			FOR_UNITS_ON (uti, is->unit_menu_tile)
				if (! Main_Screen_Form_is_unit_hidden_from_player (p_main_screen_form, __, uti.unit)) {
					struct unit_to_list utl = {
						.id = uti.id,
						.type_id = uti.unit->Body.UnitTypeID,
						.moves_remaining = Unit_get_max_move_points (uti.unit) - uti.unit->Body.Moves,
						.damage = uti.unit->Body.Damage
					};
					if (is->count_units_to_list >= is->units_to_list_capacity) {
						int new_capacity = not_below (10, 10 * is->units_to_list_capacity);
						struct unit_to_list * new_list = malloc (new_capacity * sizeof *new_list);
						if (is->units_to_list != NULL) {
							for (int n = 0; n < is->count_units_to_list; n++)
								new_list[n] = is->units_to_list[n];
							free (is->units_to_list);
						}
						is->units_to_list = new_list;
						is->units_to_list_capacity = new_capacity;
					}
					is->units_to_list[is->count_units_to_list] = utl;
					is->count_units_to_list++;
				}

			int count_new_entries = not_above (0x100 - this->Item_Count, is->count_units_to_list);
			int unit_append_index = this->Item_Count; // index of the first appended unit

			for (int n = 0; n < count_new_entries; n++) {
				Unit_Body const * unit_body = p_units->Units[is->units_to_list[n].id].Unit;
				UnitType const * unit_type = &p_bic_data->UnitTypes[unit_body->UnitTypeID];

				// examples from the base game:
				//   Wake Veteran (4/4) Bomber (0(12).2)
				//   Wake Regular (3/3) Transport (1.2.7/7)

				// TODO: Load label in case of other languages
				char const * wake_or_activate = (unit_body->UnitState & UnitState_Fortifying) ? "Wake" : "Activate";

				char str[30];
				if (unit_type->Bombard_Strength > 0)
					snprintf (str, sizeof str, "%d(%d)", unit_type->Attack, unit_type->Bombard_Strength);
				else
					snprintf (str, sizeof str, "%d", unit_type->Attack);
				str[(sizeof str) - 1] = '\0';

				char str_and_moves[30];
				int remaining_movement = 0;
				snprintf (str_and_moves, sizeof str_and_moves, "%s.%d.%d/%d", str, unit_type->Defence, remaining_movement, unit_type->Movement);
				str_and_moves[(sizeof str_and_moves) - 1] = '\0';

				char s[300];
				snprintf (s, sizeof s, "%s %s (%s)", wake_or_activate, unit_type->Name, str_and_moves);
				s[(sizeof s) - 1] = '\0';
				Context_Menu_add_item (this, __, 0x13 + p_bic_data->UnitTypeCount + is->units_to_list[n].id, s, 0, 0);
			}

			// Move appended units into their proper position
			if (count_new_entries > 0) {
				int count_entries_to_table = this->Item_Count - count_new_entries;
				if (count_entries_to_table > is->cm_temp_items_capacity) {
					int new_capacity = not_below (10, count_entries_to_table * 2);
					Context_Menu_Item * new_list = malloc (new_capacity * sizeof *new_list);
					if (is->cm_temp_items != NULL)
						free (is->cm_temp_items);
					is->cm_temp_items = new_list;
					is->cm_temp_items_capacity = new_capacity;
				}
				for (int n = 0; n < count_entries_to_table; n++)
					is->cm_temp_items[n] = this->Items[unit_start_index + n];
				int j = unit_start_index = unit_start_index;
				for (int n = 0; n < count_new_entries; n++)
					this->Items[j++] = this->Items[unit_append_index + n];
				for (int n = 0; n < count_entries_to_table; n++)
					this->Items[j++] = is->cm_temp_items[n];
			}
		}
