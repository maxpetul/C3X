...
patch_Leader_clear_at_game_start (Leader * this, int edx, int race_id)
{
	...
	// Set RaceID field to 1 after clearing the Leader object (which is what the call above does; race_id is set to -1 at the call site that gets
	// redirected here). If we don't do this the RaceID field will be left as -1.
	this->RaceID = 1;
}
